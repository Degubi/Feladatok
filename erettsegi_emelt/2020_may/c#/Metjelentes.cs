...
for(var i = adatok.Count - 1; i >= 0; --i) {
    ...
    if(jelenlegi.telepules == bekertKod) {
        Console.WriteLine($"Utolsó mérés időpontja: {jelenlegi.ido.ToShortTimeString()}");
        break;
    }
}
...
Console.WriteLine($"Legalacsonyabb hőmérséklet: {minHomersekletes.telepules} {minHomersekletes.ido.ToShortTimeString()} {minHomersekletes.homerseklet} fok");
Console.WriteLine($"Legmagasabb hőmérséklet: {maxHomersekletes.telepules} {maxHomersekletes.ido.ToShortTimeString()} {maxHomersekletes.homerseklet} fok");
...
if(szelcsendek.Count == 0){
    ...
    foreach(var csendes in szelcsendek) {
        Console.WriteLine($"{csendes.telepules}: {csendes.ido.ToShortTimeString()}");
    }
}
...
foreach(var (telepules, telepuleshezTartozoAdatok) in adatokTelepulesenkent) {
    ...
    if(egyediAdattalRendelkezoOrak.Count == 24) {
        ...
        var kerekitettKozep = (int) Math.Ceiling(kozep);
