...
var orderedVersenyzok = versenyzok.OrderByDescending(k => k.pontok).ToList();

Console.WriteLine("7. feladat: A verseny legjobbjai:");
for(int k = 1, index = 0; k < 4; ++k, ++index) {
    var versenyzo = orderedVersenyzok[index];

    Console.WriteLine(k + ". díj "+ versenyzo);
    if(versenyzo.pontok == orderedVersenyzok[index + 1].pontok) {
        Console.WriteLine(k + ". díj "+ orderedVersenyzok[++index + 1]);
    }
}
