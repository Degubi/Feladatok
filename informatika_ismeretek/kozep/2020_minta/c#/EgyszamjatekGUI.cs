...
int TippekSzama(string tippSzoveg) {
    return tippSzoveg.Length == 0 ? 0 : tippSzoveg.Split(' ').Length;
}

TextBox NewTextBox(string labelText, int y, int width, Form form) {
...
Label NewLabel(string text, int x, int y, int width, Form form) {
    var label = new Label { Text = text, Location = new Point(x, y), Size = new Size(width, 13) };
    form.Controls.Add(label);
    return label;
}

Button NewButton(string text, int x, int y, int width, Form form) {
